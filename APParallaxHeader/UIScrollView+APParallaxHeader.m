...
- (id)initWithFrame:(CGRect)frame andShadow:(BOOL)shadow {
    if(self = [super initWithFrame:frame]) {
        ...
        [self.imageView addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(imageViewDidTap)]];
...
{
    if ([self.delegate respondsToSelector:@selector(parallaxView:imageViewDidTap:)]) {
        [self.delegate parallaxView:self imageViewDidTap:self.imageView];
    }
}
